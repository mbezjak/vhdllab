...
/**
 * This class represents a registered method for "generate testbench VHDL" request.
 * 
 * @author Miro Bezjak
 */
public class DoMethodGenerateTestbenchVHDL implements JavaToAjaxRegisteredMethod {
	...
	public Properties run(Properties p, VHDLLabManager labman) {
		...
		// Generate testbench VHDL
		String vhdl = null;
		try {
			File file = labman.loadFile(id);
			vhdl = labman.generateTestbenchVHDL(file);
		} catch (ServiceException e) {
			vhdl = null;
		}
		if(vhdl==null) return errorProperties(MethodConstants.SE_CAN_NOT_GENERATE_VHDL_TESTBENCH,"Can not generate testbench VHDL!");

		// Prepare response
		Properties resProp = new Properties();
		resProp.setProperty(MethodConstants.PROP_METHOD,MethodConstants.MTD_GENERATE_TESTBENCH_VHDL);
