...
/**
 * This class describes one signal of {@link Generator inducement}. It consists
 * of signal name, range (signal type) and a {@linkplain java.util.List} of
 * {@linkplain Impulse} also known as exciter.
 * 
 * <h3>Example</h3>
 * 
 * Here is an example of declaring a signal in inducement: <blockquote>
 * &lt;signal name="a"
 * type="scalar"&gt;(0,0)(100,1)(300,0)(400,1)(800,0)&lt;/signal&gt;
 * </blockquote> In this example this class will contain signal name:
 * <code>a</code>, range that describes signal as a scalar (<code>null</code>
 * value) and a list of impulses that describes this particular exciter.
 * Therefor one impulse in an exciter will contain information about the
 * following impuse: <i>(0,0)</i>! This is true for every other impulse in
 * exciter.
 * <p>
 * Another example, but with declaring a vector: <blockquote> &lt;signal
 * name="b" type="vector" rangeFrom="2"
 * rangeTo="0"&gt;(0,000)(200,001)(300,010)(400,100)(800,101)(900,111)&lt;/signal&gt;
 * </blockquote> In this example this class will contain signal name:
 * <code>b</code>, range that describes vector bounds: 2, 0 and a list of
 * impulses that describes this particular exciter. Therefor one impulse in an
 * exciter will contain information about the following impuse: <i>(0,000)</i>!
 * This is true for every other impulse in exciter.
 * 
 * <h3>Restrictions</h3>
 * 
 ...
 * uniformly describing signal as a scalar and when describing a vector, it will
 * contain exactly two elemets: index of lower bound of a vector (rangeFrom) and
 * index of upper bound of a vector (rangeTo) both of which will not be negative
 * numbers thus uniformly describing bounds of a vector.
 * <p>
...
public class DefaultSignal implements Signal {
	...
	/**
	 * Static field that uniformly describes signal as a scalar by setting
	...
	/**
	 * Signal range. It contains either information that signal is a scalar (by
	...
	/**
	 * Create an instance of this class using only signal name to describe a
	 * signal. Exciter will remain empty and range will be set so that it
	 * describes this signal as a scalar (to value {@linkplain #SCALAR_RANGE}).
	 * 
	 * <h3>Restrictions</h3>
	 * 
	 ...
	 *             <li>at least one element of <code>range</code> is
	 *             negative.
	 *             </ul>
	 */
	...
	/**
	 * Create an instance of this class using signal name and exciter to
	 * describe a signal. Exciter's contents will be added to intern exciter.
	 * Range will be set so that it describes this signal as a scalar (to value
	 * {@linkplain #SCALAR_RANGE}).
	 * 
	 * <h3>Restrictions</h3>
	 * 
	 * Signal name must have the following format:
	 ...
	 * </ul>
	 * <p>
	 * Every impulse in exciter that does not abide to following rules will not
	 * be added to internal exciter:
	 * <p>
	 * An exciter must not contain two or more
	 * {@link Impulse#equals(Object) equal} impulses and it must be sorted
	 * ascending by time. Two or more neighborly impulses in exciter must not
	 * share same state. Also state lenght (number of bits) must match range.
	 * For example:
	 * <ul>
	 * <li>if signal is a scalar then every impulse in exciter must contain
	 * state that has only one bit
	 * <li>if signal is a vector then every impulse in exciter must contain
	 * state that has exactly <code>Math.abs(rangeFrom - rangeTo + 1)</code>
	 * bits
	 * </ul>
	 * 
	 * @param name
	 *            a name of a signal.
	 * @param exciter
	 *            a list of impulses that will be added.
	 * @throws NullPointerException
	 *             if <code>name</code> or <code>exciter</code> is
	 *             <code>null</code>.
	 * @see #addExciter(List)
	 */
	public DefaultSignal(String name, List<Impulse> exciter) {
		if (exciter == null)
			throw new NullPointerException("Exciter can not be null.");
	...
	/**
	 * Create an instance of this class using signal name and impulse to
	 * describe a signal. Impulse will be added to intern exciter. Range will be
	 * set so that it describes this signal as a scalar (to value
	 * {@linkplain #SCALAR_RANGE}).
	 * 
	 * <h3>Restrictions</h3>
	 * 
	 ...
	 * </ul>
	 * <p>
	 * An impulse will not be added if state lenght of does not match range, for
	 * example:
	 * <ul>
	 ...
	 *            a name of a signal.
	 * @param impulse
	 *            an impulse that will be added.
	 * @throws NullPointerException
	 *             if <code>name</code> or <code>impulse</code> is
	 *             <code>null</code>.
	 * @see #addImpulse(Impulse)
	 */
	public DefaultSignal(String name, Impulse impulse) {
		if (impulse == null)
			throw new NullPointerException("Impulse can not be null.");
	 ...
	 * @see #addExciter(List)
	 */
	private void createDefaultSignal(String name, int[] range,
			List<Impulse> exciter) {
		if (name == null)
		...
		if (range == null)
			this.range = DefaultSignal.SCALAR_RANGE;
		else
			this.range = new int[] { range[0], range[1] };
		if (exciter != null)
			this.addExciter(exciter);
	}
	...
	/**
	 * Returns a list of impulses. Returned list is sorted according to the way
	 * impulses were added. A returned list of impulses is read-only to ensure
	 * consistency!
	 * 
	...
	public boolean equals(Object o) {
		...
		if (!(other.getName().equalsIgnoreCase(this.signalName)
				&& other.getExciter().size() == this.exciter.size() && other
				.getExciter().equals(this.exciter)))
			return false;
	 ...
	 * <code>ds1.hashCode() == ds2.hashCode()</code> for any two
	 * DefaultSignals, <code>ds1</code> and <code>ds2</code>, as required
	...
	public int hashCode() {
		if (isVector())
			return signalName.toLowerCase().hashCode()
					^ Integer.valueOf(getRangeFrom()).hashCode()
					^ Integer.valueOf(getRangeTo()).hashCode()
					^ exciter.hashCode();
		else
			return signalName.toLowerCase().hashCode() ^ exciter.hashCode();
	}

	/**
	 * Returns a string representing detailed description of this
	 * <code>DefaultSignal</code> instance. Returned string will have the
	 * following format:
	 * <p>
	 * SIGNAL NAME: --signal name here--, RANGE: --string representing range and
	 * vector direction here--, CONTAINS --exciter size here-- IMPULSES:<br>
	 * --string representing every impulse in impulse list here--
	 * 
	 * @return a string representing detailed description of this
	 *         <code>DefaultSignal</code> instance.
	 */
	@Override
	public String toString() {
		StringBuilder retval = new StringBuilder(30 + signalName.length()
				+ exciter.toString().length());
		...
		if (isScalar())
			...
			if (getRangeFrom() >= getRangeTo())
				retval.append(" DOWNTO ");
			else
				retval.append(" TO ");
			retval.append(getRangeTo());
		}
		retval.append(", CONTAINS ").append(exciter.size()).append(
				" IMPULSES:\n");
	 ...
	 * <h3>Restrictions</h3>
	 * 
	 * An impulse <code>i</code> will not be added if one of the following is
	 * true:
	 * <ul>
	 ...
	 * <li>if state lenght of <code>i</code> does not match range, for
	 * example:
	 * <ul>
	 * <li>if signal is a scalar then <code>i</code> must contain state that
	 * has only one bit
	...
	public boolean addImpulse(Impulse i) {
		if (i == null)
			throw new NullPointerException("Impulse can not be null.");
		if (isScalar() && i.getState().length() != 1)
			return false;
		if (isVector()
				&& (Math.abs(this.getRangeFrom() - this.getRangeTo()) + 1) != i
						.getState().length())
			return false;

		if (!exciter.isEmpty()) {
			Impulse lastImpulse = exciter.get(exciter.size() - 1);
			if (lastImpulse.getTime() > i.getTime())
				return false;
			if (lastImpulse.getState().equalsIgnoreCase(i.getState()))
				return false;
		}
		exciter.add(i);
	 ...
	 * Impulses from <code>exciter</code> will be added one at a time. Every
	 * impulse in exciter that does not abide to following rules will not be
	 ...
	 * 
	 * @param exciter
	 *            an exciter that will to be added.
	 * @return <code>true</code> if at least one impulse in
	 *         <code>exciter</code> has been added; <code>false</code>
	 *         otherwise.
	 * @throws NullPointerException
	 *             if <code>exciter</code> is <code>null</code>.
	 * @see #addImpulse(Impulse)
	 */
	 ...
	 * <code>time</code>. In order to find an impulse at time 0 it is
	 * possible for <code>time</code> to be <code>-1</code>. In that case
	 * this method will find first impulse in exciter (most likely at time 0).
	 * (Other then <code>-1</code>, <code>time</code> must not be negative)
	 * 
	 * @param time
	 *            a time from where the search begins.
	 * @return an impulse whose time is greater then <code>time</code>;
	 *         <code>null</code> if that impulse does not exist.
	 * @throws IllegalArgumentException
	 *             if <code>time</code> is less then <code>-1</code>.
	 * @see Impulse
	 */
	public Impulse getImpulseAfterTime(long time) {
		if (time < -1)
			throw new IllegalArgumentException("Time can not be less then -1.");

		if (exciter.size() == 0)
			return null;
		if (time == -1)
			return exciter.get(0);
		int index = exciter.indexOf(getImpulseAtTime(time)) + 1;
		if (index >= exciter.size())
			return null;
	 ...
	 * @return an impulse that contains <code>time</code>; <code>null</code>
	 *         if that impulse does not exist.
	 * @throws IllegalArgumentException
	 *             if <code>time</code> is <code>negative</code>.
	 * @see Impulse
	 */
