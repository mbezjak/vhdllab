...
public class Testbench implements IVHDLGenerator {
	...
	public VHDLGenerationResult generateVHDL(File f, VHDLLabManager labman)
		...
		if (!f.getFileType().equals(FileTypes.FT_VHDL_TB))
	...
	private void populateComponent(CircuitInterface ci, StringBuilder vhdl) {
		vhdl.append("component ").append(ci.getEntityName()).append(
				" is port (\n");
		for (Port p : ci.getPorts()) {
			...
			if (p.getType().isVector())
				vhdl.append("(").append(p.getType().getRangeFrom()).append(" ")
						.append(p.getType().getVectorDirection()).append(" ")
