...
/**
 * This class contains various method for Testbench VHDL generation.
 * 
 * @author Miro Bezjak
 */
public class Testbench implements VHDLGenerator {
	 ...
	 *            a circuit interface.
	 * @param generator
	 *            a generator representing inducement.
	 * @return a string representing testbench VHDL.
	 * @throws NullPointerException
	 *             if <code>ci</code> or <code>generator</code> is
	 *             <code>null</code>.
	 * @throws ParseException
	 *             if inducement is not of correct format.
	 * @throws IncompatibleDataException
	 *             if <code>ci</code> is not compatible with
	 *             <code>generator</code>.
	 * @see CircuitInterface
	 * @see CircuitInterface#isCompatible(Generator)
	 * @see Generator
	 */
	private String writeVHDL(CircuitInterface ci, Generator generator,
			String sourceName) throws ParseException, IncompatibleDataException {
		if (ci == null)
			throw new NullPointerException("Circuit interface can not be null.");
		if (generator == null)
			throw new NullPointerException("Generator can not be null.");
		if (!generator.isCompatible(ci))
			throw new IncompatibleDataException(
					"Circuit Interface's ports are not compatible with Generator's signals.");

		StringBuilder vhdl = new StringBuilder(generator.toString().length());
		vhdl.append("library IEEE;\n").append("use IEEE.STD_LOGIC_1164.ALL;\n")
				.append("\n").append("entity ").append(sourceName).append(
						" is \n").append("end ").append(sourceName).append(
						";\n").append("\n").append(
						"architecture structural of ").append(sourceName)
				.append(" is \n");
	...
	private void populateComponent(CircuitInterface ci, StringBuilder vhdl) {
		vhdl.append("component ").append(ci.getName()).append(
				" is port (\n");
		for (Port p : ci.getPorts()) {
			...
			if (p.getType().getRange().isVector())
				vhdl.append("(").append(p.getType().getRange().getFrom()).append(" ")
						.append(p.getType().getRange().getDirection()).append(" ")
						.append(p.getType().getRange().getTo()).append(")");
			vhdl.append(";\n");
		}
	 ...
	 *            a string builder where to store testbench VHDL.
	 */
	private void populateProcess(CircuitInterface ci, Generator gen,
			StringBuilder vhdl) {
		List<Slot> list = createProcessList(ci, gen);
		long time = -1;
		for (Slot slot : list) {
			if (time != -1) {
				vhdl.append("\t\twait for ").append(slot.getTime() - time)
						.append(" ").append(gen.getMeasureUnit()).append(
								";\n\n");
			}
			for (ImpulseWithSignalName imp : slot.getList()) {
				String state = imp.getImpulse().getState();
				vhdl.append("\t\t").append(
						getTestbenchSignal(imp.getSignalName())).append(" <= ");
				if (ci.getPort(imp.getSignalName()).getType().getRange().isScalar())
	...
	/**
	 * Create a process list. This method will create a list of Slot from where
	 * method populateProcess(CircuitInterface, Generator, StringBuilder) will
	 * draw information regarding signal changes written in Generator.
	 * 
	 * @param ci
	 *            a circuit interface from where to draw information.
	 * @param gen
	...
	private List<Slot> createProcessList(CircuitInterface ci, Generator gen) {
		...
		for (Signal s : gen.getSignals()) {
			...
			for (Impulse i : s.getExciter()) {
				...
				ImpulseWithSignalName imp = new ImpulseWithSignalName(s
						.getName(), i);
	 ...
	 * </blockquote> In this example this class will contain information that in
	 * time 0 signals 'A', 'b' and 'c' changed their state to '0'. This is true
	 * for every other signal change in this inducement.
	 * 
	 * <h3>Restrictions</h3>
	 * 
	 * Time will be non-negative number and a list of ImpulseWithSignalName will
	 * not contain two {@link Testbench.ImpulseWithSignalName#equals(Object)
	...
	private class Slot implements Comparable<Slot> {
		...
		private List<ImpulseWithSignalName> list_ro = Collections
				.unmodifiableList(list);

		/**
		 * Construct an instance of this class using a time to explain a Slot. A
		 ...
		 * 
		 * @param i
		 *            an ImpulseWithSignalName to add.
		...
		public boolean addImpulseWithSignalName(ImpulseWithSignalName i) {
			if (i == null)
				throw new NullPointerException(
						"ImpulseWithSignalName can not be null.");
		 ...
		 *         argument; and a value greater than 0 if this slot has time
		 *         greater than the slot argument.
		 * @throws ClassCastException
		 *             if the specified object's type prevents it from being
		 *             compared to this Slot.
		 */
		...
		/**
		 * Compares this slot to the specified object. Returns <code>true</code>
		 * if and only if the specified object is also a <code>Slot</code> and
		 * if times are the same.
		 * 
		 * @param o
		 *            the object to compare this <code>Slot</code> against.
		 * @return <code>true</code> if <code>Slot</code> are equal;
		 *         <code>false</code> otherwise.
		 */
		@Override
		...
		/**
		 * Returns a hash code value for this <code>Slot</code> instance. The
		 * hash code of <code>Slot</code> instance is hash code of time.
		 * <p>
	 ...
	 * &lt;signal name="d" type="vector" rangeFrom="0"
	 * rangeTo="0"&gt;(100,1)(200,0)(300,1)(400,z)&lt;/signal&gt;<br>
	 * </blockquote> In this example this class will contain information that
	 * signal A is changing its state to a cirtain impulse <i>(0,0)</i>. This
	 * is true for every other signal and impulse in this inducement.
	 * 
	 * <h3>Restrictions</h3>
	 * 
	...
	private class ImpulseWithSignalName {
		...
		/**
		 * Construct an instance of this class using a signal name and an
		 * impulse to explain an ImpulseWithSignalName. A signal name must have
		 ...
		 *            a name of a signal.
		 * @param impulse
		 *            an impuse to which signal is changing.
		 * @throws NullPointerException
		 ...
		 *            <code>ImpulseWithSignalName</code> against.
		 * @return <code>true</code> if <code>ImpulseWithSignalName</code>
		...
		public boolean equals(Object o) {
			if (o == null)
				return false;
			if (!(o instanceof ImpulseWithSignalName))
				return false;
    ...
    public VHDLGenerationResult execute(File file) throws ServiceException {
        ...
        ServiceManager labman = ServiceContainer.instance().getServiceManager();
        ...
        String vhdl = null;
        try {
            Generator gen = new DefaultGenerator(file.getContent());
            vhdl = writeVHDL(ci, gen, file.getName());
        } catch (Exception e) {
            throw new ServiceException(StatusCodes.SERVICE_CANT_GENERATE_VHDL_CODE, e.getMessage());
        }
        return new VHDLGenerationResult(true, new ArrayList<VHDLGenerationMessage>(0), vhdl);
    }
