...
/**
 * A run dialog that displays to a user list of files and he should select one.
 * To use this dialog just use one constructor, modify text to buttons
 * (optional) and just invoke method {@link #startDialog()}. After that
 * controls are locked and youre code will stop at that method. When user clicks
 * on a button (OK, CANCEL or CLOSE) controls will become unlocked and youre
 * code will resume just after {@link #startDialog()} method. To know what
 * button user clicked use {@link #getOption()} method. Finaly to get what file
 * user selected use {@link #getSelectedFile()} method. This dialog uses project
 * container to get files to display and dialog type to chose which method to
 * invoke. So far these are dialog types:
 * <ul>
 * <li>{@link #COMPILATION_TYPE} - uses
 * {@link ISystemContainer#getAllCircuits(String)} method</li>
 * <li>{@link #SIMULATION_TYPE} - uses
 * {@link ISystemContainer#getAllTestbenches(String)} method</li>
 * </ul>
 * <p>
 * Here is an example of how to user this dialog:
 * </p>
 * <code>
 * <pre>
...
public class RunDialog extends JDialog {
	 ...
	 * 
	 * @param owner
	 *            the <code>Component</code> from which the dialog is
	 *            displayed.
	 * @param modal
	 ...
	 *            system container to enable communication
	 * @param dialogType
	 *            a type of this dialog
	 * @exception HeadlessException
	 *                if GraphicsEnvironment.isHeadless() returns true.
	 * @see java.awt.GraphicsEnvironment#isHeadless
	 * @see JComponent#getDefaultLocale
	 */
	public RunDialog(Component owner, boolean modal,
			ISystemContainer container, int dialogType)
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public RunDialog(Frame owner, boolean modal, ISystemContainer container,
			int dialogType) throws HeadlessException {
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public RunDialog(Dialog owner, boolean modal, ISystemContainer container,
			int dialogType) throws HeadlessException {
		super(owner, null, modal);
		runDialogImpl(owner, container, dialogType);
	}
	...
	/**
	 * Creates a modal or non-modal dialog without a title and with the
	 * specified owner <code>JApplet</code>. If <code>owner</code> is
	 * <code>null</code>, a shared, hidden frame will be set as the owner of
	 * the dialog.
	 * <p>
	 * This constructor sets the component's locale property to the value
	 ...
	 * 
	 * @param owner
	 *            the <code>JApplet</code> from which the dialog is displayed
	 * @param modal
	 *            true for a modal dialog, false for one that allows others
	 *            windows to be active at the same time
	 * @param container
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public RunDialog(JApplet owner, boolean modal, ISystemContainer container,
			int dialogType) throws HeadlessException {
	 ...
	 * 
	 * @param owner
	 *            owner for which the dialog is displayed
	 * @param container
	 *            system container to enable communication
	 * @param dialogType
	 *            a type of this dialog
	 */
	private void runDialogImpl(Component owner, ISystemContainer container,
			int dialogType) {
		this.owner = owner;
		...
		Border outsideProjectBorder = BorderFactory.createEmptyBorder(BORDER,
				BORDER, BORDER, BORDER);
		currentProjectBorder = BorderFactory
				.createTitledBorder("Displayed Project");
		currentProjectPanel.setBorder(BorderFactory.createCompoundBorder(
				outsideProjectBorder, currentProjectBorder));
	...
	/**
	 * Returns file identifier that was selected by user to run. This method
	 * will return <code>null</code> if user did not click on a OK button.
	 * 
	...
	/**
	 * Set text to be displayed in a current project label just above a list of
	 * files. If <code>text</code> is <code>null</code> then current project
	...
	/**
	 * Set text to be displayed in a change project button, left to current
	 * project label. If <code>text</code> is <code>null</code> then change
	 * project button will be set to an empty string.
	 * 
	 * @param text
	 *            text to be displayed
	 */
	...
	/**
	 * Set text to be displayed as a title of file list. If <code>text</code>
	 * is <code>null</code> then list title will be set to an empty string.
	 * 
	 * @param text
	 *            text to be displayed
	 */
	 ...
	 * <code>text</code> is <code>null</code> then button will be set to an
	 * empty string.
	 * 
	 * @param text
	 *            text to be displayed
	...
	/**
	 * Preforms action to be taken after user wants to exist this dialog with
	 * {@link #OK_OPTION}.
	 */
	 ...
	 * 
	 * @param invoker
	 *            component that is invoking this popup
	 */
	private void preformPopupMenuAction(Component invoker) {
		int x = 0;
		int y = 0;
		int difference = invoker.getSize().width
		...
		try {
			if (currentProject != null) {
				if (dialogType == RunDialog.COMPILATION_TYPE) {
					files = container.getResourceManager().getAllCircuits(
							currentProject);
				} else if (dialogType == RunDialog.SIMULATION_TYPE) {
					files = container.getResourceManager().getAllTestbenches(
							currentProject);
				}
			}
	...
	/**
	 * Creates and positions an owner user by dialog to ensure that dialog is
	 * modal and centered over applet.
	 * 
