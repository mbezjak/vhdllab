...
*/
	...
	public void setData(CircuitInterface data) {
		...
		for(Port p:lista){
			obj[i][0]=p.getName();
			if(p.getDirection().equals(PortDirection.IN))obj[i][1]="in"; 
				else obj[i][1]="out";
			Type tip=p.getType();
	...
	public CircuitInterface getCircuitInterface() {
		...
		for(int i=0;i<pomData.length;i++){
			...
			if(pomData[i][1].toLowerCase().equals("in"))d=PortDirection.IN; else d=PortDirection.OUT;
			Type tip=null;
			if(pomData[i][2].equalsIgnoreCase("Std_Logic"))tip=new Type(TypeName.STD_LOGIC, Range.SCALAR);
			else {
				if(Integer.parseInt(pomData[i][3])>Integer.parseInt(pomData[i][4])){
					...
					tip=new Type(TypeName.STD_LOGIC_VECTOR,new Range(inte[0],VectorDirection.DOWNTO, inte[1]));
				}else{
					int[] inte={Integer.parseInt(pomData[i][3]),Integer.parseInt(pomData[i][4])};
                    tip=new Type(TypeName.STD_LOGIC_VECTOR,new Range(inte[0],VectorDirection.TO, inte[1]));
				}
			}
